import pygame
from settings import tile_size, WIDTH, HEIGHT, others, DESERT, RED, level_map, VIOLET
...
class Level: # szintek önálló osztály, nem sprite osztály
    ...
    def horizontal_movement_collision(self):
        ...
        if player.rect.right>WIDTH: #ha a játékos jobb oldala nagyobb mint a képernyő szélessége
            player.rect.right=WIDTH
        if player.rect.right> (WIDTH/5)*4:
            self.breakTheRules()
        ...
        for enemy in self.enemys: #ellenségeket szétszedem a csoportból
            if player_rect.colliderect(enemy.rect): #ha ütköznek
                ...
                if player_rect.bottom==enemy.rect.bottom and self.player.sprite.on_ground: #ha a játékos és az ellen egy szinten a földön ütköznek
                    self.player.sprite.mini_jump() #akkor veszít
                    self.health-=1
    ...
    def statsOnScreen(self): #életerő és pontok kiiratása
        ...
        text=font.render(f'Health: {self.health} Points: {self.points} Level: {self.level+1}', True, VIOLET) #mit
        text_rect=text.get_rect(topleft=(40,50)) #hova
        self.display_surface.blit(text,text_rect) #megjelenítés

    def breakTheRules(self): #életerő és pontok kiiratása
        font=self.setup_font(18) #betűtípus és méret
        text=font.render(f'Break the rules << Turn left & collect the easter eggs!', True, RED) #mit
        text_rect=text.get_rect(center=(WIDTH//2,HEIGHT//5)) #hova
        self.display_surface.blit(text,text_rect) #megjelenítés
    ...
    def end_game(self): #játék vége
        ...
        if self.enemys.sprites()==[] or self.crates.sprites()==[] and self.player.sprite.on_ground: #ha már nincs ellenség
            background.stop() #zene leáll
            win.play() #vége hang
            background.play(-1) #zene indul
